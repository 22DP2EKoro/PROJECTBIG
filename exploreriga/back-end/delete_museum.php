...
$stmt->close();
if (!$row) { echo json_encode(["error" => "Museum not found"]); exit; }
$id = $row['id'];

$s = $conn->prepare("DELETE FROM museums WHERE objects_id = ?");
$s->bind_param("i", $id); $s->execute(); $s->close();

$s2 = $conn->prepare("DELETE FROM objects WHERE id = ?");
$s2->bind_param("i", $id); $s2->execute(); $s2->close();

echo json_encode(["success" => "Museum deleted successfully"]);
$conn->close();
?>
